...
// ================================================================ 5 · what he sees: the map
{
  ...
  legend(s, [
    ...
    ['ההטבות שלי', 'אילו הטבות כבר שולמו, מהי היתרה שנותרה לניצול, ומעקב אחר סטטוס הטיפול בבקשות שהוגשו.', '8E3BB5'],
    ['הטבות פוטנציאליות', 'מגוון הטבות שייתכן שמגיעות לך, עם אפשרות להגיש בקשה מהירה לבדיקת הזכאות.', '2A9BD6'],
  ], { x: 9.4, y: 1.85, w: W - M - 9.4, gap: 1.14 });
  s.addText('שני ההסברים התחתונים הם הנוסח שהמצפן עצמו מציג למבוטח.', {
    ...rtl, x: 9.4, y: 6.15, w: W - M - 9.4, h: 0.4, fontSize: 12, color: '93A79B',
  });
  s.addNotes('אפשר להצביע על המסך ולעבור אזור אזור. השמות הם השמות שהמבוטח יגיד בטלפון.');
}

// ================================================================ 6 · my benefits, level one
{
  const s = page('ההטבות שלי', 'רמה ראשונה: מה שרואים ברשימה, לפני שנכנסים פנימה');
  shot(s, 'benefit.png', { x: M, y: 1.58, w: 8.5, h: 4.7 });
  const X = 9.55, CW = W - M - 9.55;
  s.addText('בלשון המצפן', { ...rtl, x: X, y: 1.72, w: CW, h: 0.3, fontSize: 12, bold: true, color: '93A79B' });
  card(s, { x: X, y: 2.05, w: CW, h: 1.55 });
  s.addText('אילו הטבות כבר שולמו, מהי היתרה שנותרה לניצול, ומעקב אחר סטטוס הטיפול בבקשות שהוגשו.', {
    ...rtl, x: X + 0.25, y: 2.05, w: CW - 0.5, h: 1.55, fontSize: 14.5, color: INK, valign: 'middle',
  });
  s.addText('ומה מופיע בכל שורה', { ...rtl, x: X, y: 3.78, w: CW, h: 0.3, fontSize: 12, bold: true, color: '93A79B' });
  ['סטטוס ההטבה', 'תקופת הזכאות', 'סכום ההטבה', 'כמות', 'סוג התשלום', 'כפתור "פרטים נוספים"']
    .forEach((t, i) => {
      const y = 4.12 + i * 0.42;
      s.addShape(pres.ShapeType.ellipse, { x: X + CW - 0.2, y: y + 0.13, w: 0.12, h: 0.12, fill: { color: NAVY } });
      s.addText(t, { ...rtl, x: X, y, w: CW - 0.35, h: 0.38, fontSize: 15, color: INK, valign: 'middle' });
    });
  card(s, { x: M, y: 6.42, w: W - 2 * M, h: 0.5, fill: 'FBEFE6', line: CLAY, lw: 1.4 });
  s.addText('ב"ההטבות שלי" מוצגות תכניות מהשנתיים האחרונות בלבד.', {
    ...ctr, x: M, y: 6.42, w: W - 2 * M, h: 0.5, fontSize: 16, bold: true, color: CLAY, valign: 'middle' });
  s.addNotes('זו הרמה הראשונה. כל מה שמופיע כאן נמצא בצילום המסך שעל השקף. היתרה למימוש והסטטוסים המלאים נמצאים רק אחרי לחיצה על "פרטים נוספים".');
}
...
// ================================================================ 7 · statuses as a path
{
  ...
  const NW = 2.55, GAP = 0.55, Y = 1.72;
  const xOf = i => W - M - NW - i * (NW + GAP);
  s.addShape(pres.ShapeType.rect, { x: xOf(3) + NW / 2, y: Y + 0.29, w: xOf(0) - xOf(3), h: 0.02, fill: { color: LINE } });
  ORDER.forEach((k, i) => {
    const x = xOf(i);
    pill(s, { key: k, x, y: Y, w: NW });
    s.addText(LINES[k], { ...rtl, x, y: Y + 0.78, w: NW, h: 0.85, fontSize: 13, color: MUTED, valign: 'top' });
    if (i < 3) arrow(s, x - GAP + 0.10, Y + 0.19);
  });
  const bx = xOf(1);
  s.addShape(pres.ShapeType.rect, { x: bx + NW / 2 - 0.015, y: Y + 1.62, w: 0.03, h: 0.35, fill: { color: LINE } });
  pill(s, { key: 'נדחתה', x: bx, y: Y + 1.97, w: NW });
  s.addText('סיבת הדחייה מאישור ההחלטה מופיעה כאן, והמבוטח קורא אותה.', {
    ...rtl, x: bx - 3.35, y: Y + 2.02, w: 3.15, h: 0.62, fontSize: 13.5, color: MUTED, valign: 'top',
  });
  s.addText('וכך זה נראה במסך: הסטטוס פותח כל שורה של הטבה, בעמוד שנפתח מ"ההטבות שלי".', {
    ...rtl, x: M, y: 4.45, w: W - 2 * M, h: 0.4, fontSize: 15, bold: true, color: NAVY, align: 'center',
  });
  shot(s, 'status_rows.png', { x: M, y: 4.9, w: W - 2 * M, h: 2.0 });
  s.addNotes('הנימוק שנבחר באישור ההחלטה הוא הטקסט שהמבוטח קורא. שווה לומר את זה במפורש.');
}

// ================================================================ 8 · details page anatomy
{
  const s = page('"פרטים נוספים": התמונה המלאה של התכנית', 'רמה שנייה: לחיצה שנייה, מתוך שורת ההטבה, פותחת את כל מה שיש בתכנית אחת');
  ...
  s.addText('כאן, ורק כאן, מופיעים סיכום המימוש והיתרה שנותרה. ברשימה עצמה הם אינם מוצגים.', {
    ...rtl, x: M, y: 5.95, w: 5.35, h: 0.9, fontSize: 15, bold: true, color: GREEN, valign: 'top',
  });
}
...
// ================================================================ 9 · potential benefits
{
  ...
  card(s, { x: M, y: 1.75, w: W - 2 * M, h: 0.95 });
  s.addText('מגוון הטבות שייתכן שמגיעות לך, עם אפשרות להגיש בקשה מהירה לבדיקת הזכאות.', {
    ...ctr, x: M, y: 1.75, w: W - 2 * M, h: 0.95, fontSize: 18, color: INK, valign: 'middle',
  });
  s.addText('כך המצפן עצמו מסביר את האזור הזה', { ...rtl, x: M, y: 2.75, w: W - 2 * M, h: 0.3, fontSize: 12, color: '93A79B' });
  const cw = (W - 2 * M - 0.6) / 2;
  const routes = [
  ...
  routes.forEach((r, i) => {
    const x = M + (1 - i) * (cw + 0.6);
    card(s, { x, y: 3.2, w: cw, h: 2.2, line: r[1], lw: 1.6 });
    shot(s, r[2], { x: x + cw - 2.1, y: 3.42, w: 1.8, h: 0.55 });
    s.addText(r[0], { ...rtl, x: x + 0.32, y: 4.05, w: cw - 0.64, h: 0.45, fontSize: 20, bold: true, color: r[1] });
    s.addText(r[3], { ...rtl, x: x + 0.32, y: 4.5, w: cw - 0.64, h: 0.75, fontSize: 15.5, color: INK, valign: 'top' });
  });
  card(s, { x: M, y: 5.65, w: W - 2 * M, h: 1.2, fill: 'FBEFE6', line: CLAY, lw: 1.5 });
  s.addText('לתשומת ליבך', { ...rtl, x: M + 0.4, y: 5.78, w: W - 2 * M - 0.8, h: 0.32, fontSize: 14, bold: true, color: CLAY });
  s.addText('לעיתים, אותה הטבה תופיע גם ב"ההטבות שלי" וגם ב"הטבות פוטנציאליות". מצב זה קורה כאשר קיימת אפשרות להרחיב את ההטבה, למשל לבקש זכאות עבור תקופה נוספת או עבור בן משפחה נוסף.', {
    ...rtl, x: M + 0.4, y: 6.1, w: W - 2 * M - 0.8, h: 0.7, fontSize: 15, color: INK, valign: 'top',
  });
  s.addNotes('שני ההסברים על השקף הם הנוסח שהמצפן עצמו מציג למבוטח.');
}

// ================================================================ 10 · submitting an invoice
{
  const s = page('הגשת חשבונית או קבלה', 'שני מקומות שמהם אפשר להגיש');
  const cw = (W - 2 * M - 0.6) / 2;
  const froms = [
    ['מתכנית פעילה', '1F6E43', 'בדף ההטבה, בכפתור "להגשת חשבונית / קבלה חדשה".'],
    ['מהטבה פוטנציאלית', '5E3A94', 'כאשר כפתור הפעולה שם הוא "הגשת בקשה".'],
  ];
  froms.forEach((f, i) => {
    const x = M + (1 - i) * (cw + 0.6);
    card(s, { x, y: 1.8, w: cw, h: 1.5, line: f[1], lw: 1.6 });
    s.addText(f[0], { ...rtl, x: x + 0.32, y: 1.98, w: cw - 0.64, h: 0.45, fontSize: 20, bold: true, color: f[1] });
    s.addText(f[2], { ...rtl, x: x + 0.32, y: 2.45, w: cw - 0.64, h: 0.7, fontSize: 15.5, color: INK, valign: 'top' });
  });
  const steps = [
  ...
  steps.forEach((t, i) => {
    const y = 3.75 + i * 0.85;
    badge(s, M + 5.7 - 0.44, y, i + 1, i === 2 ? GREEN : NAVY, 0.44);
    s.addText(t, { ...rtl, x: M, y: y - 0.03, w: 5.1, h: 0.75, fontSize: 16.5, color: INK, valign: 'top' });
  });
  shot(s, 'emp_sent.png', { x: 6.9, y: 3.7, w: W - M - 6.9, h: 1.9 });
  card(s, { x: M, y: 6.3, w: W - 2 * M, h: 0.62, fill: 'FBEFE6', line: CLAY, lw: 1.4 });
  s.addText('המסמכים הנדרשים משתנים מהטבה להטבה ומפורטים בטופס. הודעת האישור אומרת שהפנייה נקלטה, לא שהזכאות אושרה.', {
    ...ctr, x: M + 0.3, y: 6.3, w: W - 2 * M - 0.6, h: 0.62, fontSize: 15.5, color: INK, valign: 'middle' });
}
...
{
  const s = page('דברים שיש לשים לב אליהם');
  const rules = [
    ['"ההטבות שלי" מציג שנתיים אחורה',
     'הטבה שאינה מופיעה שם אינה בהכרח הטבה שאין עליה זכאות.'],
    ['חשבונית שהוגשה לתכנית לא מתאימה',
     'תמיד פותחים את התכנית שאליה הוגשה, דוחים אותה עם הנימוק המתאים, ופותחים את התכנית הנכונה.'],
    ['נימוקי הדחייה נקראים על ידי המבוטח',
     'הנימוק שנבחר באישור ההחלטה מוצג לו כלשונו במצפן.'],
  ];
  rules.forEach((r, i) => {
    const y = 2.1 + i * 1.45;
    card(s, { x: M, y, w: W - 2 * M, h: 1.2 });
    badge(s, W - M - 0.72, y + 0.38, i + 1, NAVY, 0.48);
    s.addText(r[0], { ...rtl, x: M + 0.35, y: y + 0.18, w: W - 2 * M - 1.35, h: 0.45, fontSize: 20, bold: true, color: NAVY });
    s.addText(r[1], { ...rtl, x: M + 0.35, y: y + 0.62, w: W - 2 * M - 1.35, h: 0.5, fontSize: 15.5, color: INK, valign: 'top' });
  });
