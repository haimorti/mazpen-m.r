...
if "$FFMPEG" -hide_banner -encoders 2>/dev/null | grep libx264 >/dev/null; then
  "$FFMPEG" -y -loglevel error -f concat -safe 0 -i concat.txt -vf "fps=25,format=yuv420p" -c:v libx264 -crf 20 mazpen-guide-draft.mp4
  echo "wrote build/out/mazpen-guide-draft.mp4"
else
  "$FFMPEG" -y -loglevel error -f concat -safe 0 -i concat.txt -vf "fps=25,format=yuv420p" -c:v libvpx -b:v 2M -crf 10 mazpen-guide-draft.webm
