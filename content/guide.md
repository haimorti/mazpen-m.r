...
> המצפן נמצא כעת בשלבי בנייה והרצה. בשלב זה הוא מוצג למשפחות שכולות. אנחנו מוסיפים למצפן הטבות חדשות בהדרגה, ולכן ייתכן שהטבה מסוימת עדיין לא מופיעה בו.

---

## למי המצפן מיועד בשלב זה

בשלב זה המצפן מוצג **למשפחות שכולות** בלבד. בהמשך הוא ייפתח לאוכלוסיות נוספות, ובהן נכי איבה.

הורה שמקבל הטבות עבור **ילדים קטינים** רואה את ההטבות של הילד בתוך המצפן שלו, באזור "הצגת הטבות עבור".
...
### 3. ההטבות שלי
רשימת ההטבות שכבר ביקשתם או שכבר אושרו לכם, כל אחת ככרטיס. לחיצה על כרטיס פותחת את דף ההטבה, עם כל הפרטים והבקשות שהגשתם בה.
...
**הטבות שמתחילות בהגשת בקשה.** על הכפתור כתוב **"הגשת בקשה"**, ובכרטיס ההטבה כתוב "לבדיקת זכאות חדשה". ממלאים טופס קצר בן שלושה שלבים ומצרפים את המסמכים. הבקשה נבדקת על ידי המערכת, הבדיקה מהירה וברוב המקרים התשובה מתקבלת במהירות.

**הטבות שמתחילות בשליחת פנייה.** על הכפתור כתוב **"שליחת פנייה"**, ובכרטיס ההטבה כתוב "לבדיקת זכאות אפשר לפנות לעובד השיקום". הפנייה נשלחת לגורם המקצועי המתאים, והזכאות נבדקת מולו. הסטטוס יתעדכן במצפן.
...
### 2. מסמכים נדרשים
מצרפים את החשבונית או הקבלה, ומזינים את פרטיה. יש הטבות שדורשות קבלה בלבד, ויש שדורשות מסמכים נוספים. המסמכים הנדרשים יפורטו בטופס.
...
> **אותו טופס בשני מקומות:** טופס פרטי החשבונית שבשלב זה הוא בדיוק אותו טופס שנפתח מאוחר יותר, כשמגישים קבלה נוספת בהטבה פעילה. מי שמילא אותו פעם אחת כבר מכיר אותו.
...
| **פעילה** | ההטבה אושרה, תקופת הזכאות לא חלפה, ויש בה יתרה לניצול. | להגיש חשבוניות וקבלות להחזר. |
